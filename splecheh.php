	...
	\Carbon_Fields\Container::make( 'theme_options', __( 'Settings', 'splecheh' ) )
		...
		->add_tab(
			...
			[
				...
				\Carbon_Fields\Field::make( 'select', 'splecheh_whitespace_check', __( 'Double Spaces', 'splecheh' ) )
					->set_options(
						[
							Splecheh_SpellCheckReport::WHITESPACE_REPORT => __( 'Report as issues', 'splecheh' ),
							Splecheh_SpellCheckReport::WHITESPACE_FIX    => __( 'Fix automatically on every run', 'splecheh' ),
							Splecheh_SpellCheckReport::WHITESPACE_OFF    => __( 'Don\'t check', 'splecheh' ),
						]
					)
					->set_default_value( Splecheh_SpellCheckReport::WHITESPACE_REPORT )
					->set_help_text( __( 'What to do about runs of two or more spaces or tabs between two words. "Report as issues" lists them on the Details page for review, like spelling errors. "Fix automatically" collapses them during every spell check run without asking — the equivalent of "Fix everywhere" — and records each run in the auto-apply log; safe, since a browser collapses them anyway. "Don\'t check" ignores them entirely. Spacing inside <pre>/<code> blocks, shortcodes, HTML comments and tag attributes is never touched in any mode.', 'splecheh' ) ),
			]
		)
		->add_tab(
