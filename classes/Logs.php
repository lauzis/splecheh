...
class Splecheh_Logs {
	...
	public static function panel(): string {
		...
		if ( ! $logger || ! class_exists( '\\Lauzis\\WpPackages\\Logs\\Viewer' ) ) {
			...
			return '<p class="description">'
				. esc_html__( 'The log reader needs a newer copy of the shared package than the one running.', 'splecheh' )
				. '</p>';
		}

		$viewer = new \Lauzis\WpPackages\Logs\Viewer( $logger, [ 'clear' => 'splecheh_clear_logs' ] );
	...
	public static function handleClear(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You are not allowed to do that.', 'splecheh' ) );
		}

		check_admin_referer( 'splecheh_clear_logs' );

		self::addLog( 'logs', 'Log cleared from the settings page.', [ 'user' => get_current_user_id() ] );
		self::clearLogs();
