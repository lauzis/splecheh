...
- Interpunction Check now builds its sentence list **per block** via `Splecheh_InterpunctionReport::split_content_into_sentences()`: each block's text is split into sentences independently and concatenated in document order, so a sentence never spans a heading→paragraph, list-item→list-item, or paragraph→paragraph boundary — which previously could produce sentences impossible to write a fix back into. The Settings "Test Interpunction Check" button uses the same per-block path.
- Added a **Split Test** admin sub-page (Splecheh menu) for previewing the split: load one of several built-in example snippets (heading-next-to-paragraph, list, inline formatting, nested containers, table cells, blockquote + paragraphs) or search for a real post/page (Carbon-Fields-style autocomplete), then see the ordered block chunks with each chunk's tag, plain text, extracted sentences, and preserved inner HTML. It only previews the split — nothing is checked, fixed, or saved.
