...
### --- 0.30.0 ---
- The log now has somewhere to be read. Switching logging on wrote daily files under `uploads/splecheh-logs/` and left you to fetch them over FTP; the **Logging tab now shows them** — the days covered with entry counts and sizes, the most recent 200 lines newest first, and a button to clear them. The listing itself is the shared package's `Logs\Viewer`, since every plugin here writes the same log in the same format and each was growing its own reader for it; what stays in this plugin is whether the panel appears and the capability check behind clearing. Needs wp-plugin-packages 1.14, which also taught the settings loader to merge sections declared under the same id — without that, adding a panel beside the package's logging checkbox produced two tabs both called Logging.

### --- 0.28.0 ---
- The **background Interpunction Check now honours "Invalidate Spell Check on Plugin Version Change"**, mirroring `Splecheh_SplechehCron::version_check_sql_parts()`. The Interpunction Check table has always applied that rule to its "Outdated" badge, but the cron's post-selection query compared dates only — so with the setting on, a version bump flagged posts as Outdated that the background check would then never pick up, and "Pending" disagreed with the table for reasons nothing on screen explained. On this site that was 4 pending against 229 badged Outdated, 227 of them outdated on version alone. Pending is now 222; the remaining gap to 229 is posts held back by "Require Spell Check First", which is intended. Note the practical consequence of switching this on: every re-check is an LLM call, so a version bump queues the whole site for re-checking — turn the setting off if that isn't what you want.
- Hardened the markup-aware fix above after probing the cases its tests didn't cover: when the model **dropped a word that sat inside a tag**, the tag was hollowed out to `<em></em>` — and both existing guards passed it, since every tag survived byte-for-byte and the text still added up to the fixed sentence. A text run that had words and ends up with none now refuses the fix instead. Also pinned with tests: several tags in one sentence, nested tags, a model adding a word, the same sentence appearing twice (only the first is rewritten), HTML entities in the sentence (refused — the report holds the decoded text, the source holds the entity, so there is no safe mapping), sentences long enough to make the O(n×m) alignment expensive (refused above 2000 characters), and the known cosmetic quirk that punctuation appended to a sentence ending inside a tag lands inside it (a bold full stop). Live recovery is unchanged at 33 of 35.
- Interpunction **Fix now works on sentences that inline markup runs through** — a link, `<strong>`, `<em>` — where the sentence exists in the rendered text but never as one string in the source, so the plain and whitespace-flexible searches both failed and the fix was reported as unapplicable. `apply_fix()` gains a third stage that locates the sentence **character by character**, allowing tags between any two characters (a tag can split a word in half: `six years old</a>, kids` breaks the token "old,"), then distributes the fixed text back over the text runs by aligning it with the original (LCS), so a comma the model added stays outside the `</a>` it was typed after. Two guards, since the alternative to catching a bad edit is finding it in a published post: every tag must survive byte-for-byte, and the rewritten text runs must add up to exactly the fixed sentence — otherwise the fix is refused and reported as before. Measured against this site's live reports: of 35 markup-blocked fixes, 33 now apply; the 2 left are a model that changed the word count and one tag arrangement the pattern still can't reach.
- Made the interpunction response parser **survive the ways models actually wrap their JSON**. Both the plugin (`Splecheh_InterpunctionBackend::extract_json_array()`) and `tools/llm-wrapper.php` only stripped a ```` ``` ```` fence anchored at the very start and end of the reply, so a model that introduced its answer ("Here is the corrected JSON:") or added a closing remark left the fence markers in place and a perfectly usable reply was rejected as invalid JSON. Now a fenced block anywhere in the reply is unwrapped, and failing that the outermost `[ … ]` is extracted, dropping any prose around it. Failures also say something useful: the byte count and the start of what came back, plus — when the array is never closed — an explicit "cut off mid-generation, raise the Command Timeout or lower the Sentence Chunk Size", which is a different fix from a reply that simply isn't JSON. The raw reply is written to the interpunction log too, so the diagnosis doesn't depend on catching the on-screen error.
